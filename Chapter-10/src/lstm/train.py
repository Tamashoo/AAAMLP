...
def create_embedding_matrix(word_index, embeddings_dict):
...
def run(df, fold):
    ...
    embedding_dict = load_vectors("Chapter-10/input/crawl-300d-2M.vec")
    ...
    for epoch in range(config.EPOCHS):
        engine.train(train_dataloader, model, optimizer, device)
...
if __name__ == "__main__":
    df = pd.read_csv("Chapter-10/input/imdb_folds.csv")
    run(df, fold=0)
    run(df, fold=1)
    run(df, fold=2)
    run(df, fold=3)
